...
A second tab ("Ask the Act") offers free-form Q&A over the legal text, for
questions that don't fit the structured per-system audit flow.
"""
...
def _check_password() -> bool:
    """Option A access control: a shared password on hosted instances.
    If no APP_PASSWORD is configured (local dev), the app stays open."""
    import os
    try:
...
@st.cache_resource
def _ensure_index():
    """On a fresh host the ChromaDB index doesn't exist yet — build it once
    from the committed law text + knowledge files."""
    import chromadb
    client = chromadb.PersistentClient(path=str(config.CHROMA_DIR))
    try:
        client.get_collection(config.COLLECTION_NAME)
    except Exception:
        with st.spinner("First-time setup: building the legal knowledge base…"):
            import ingest
            ingest.main()
...
tab_audit, tab_chat = st.tabs(["📋 Audit", "💬 Ask the Act"])
...
with tab_chat:
    st.caption("Free-form questions about the EU AI Act text, grounded in the retrieved "
               "articles. For a structured per-system audit, use the Audit tab instead.")

    for msg in st.session_state.chat_history:
        with st.chat_message(msg["role"]):
            st.markdown(msg["content"])

    if q := st.chat_input("Ask anything about the EU AI Act..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("Searching the Act..."):
                try:
                    result = ask(q, history=st.session_state.chat_history[:-1])
