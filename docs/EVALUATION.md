...
**Summary.** Measures fact-extraction accuracy, duty-set precision/recall,
decision-space coverage and boundary sensitivity of the rule engine, using 50
development cases plus 30 held-out cases (never tuned against) across
NL/FR/EN/ES. On the held-out set, field accuracy is 97.1% (95% CI 91.9–99.0%,
n=104) — the only one of four target gates this sample size actually proves;
wrong-assertion rate, exact-tier accuracy, and under-warning rate are
consistent with their targets but not statistically demonstrated yet. It does
not establish legal correctness — only that the tool reads a description
consistently against one fixed reading of the Act, with no external ground
truth or lawyer-reviewed dual annotation behind it so far.
